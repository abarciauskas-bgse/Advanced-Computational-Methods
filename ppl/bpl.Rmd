...
while (length(train.labels) < 10) {
  for (i in 1:length(rand.idcs)) {
    ...
    if (!(label %in% train.labels)) {
      print(paste('adding label:', label))
...
test.set <- list()
test.labels <- c()
while (length(test.labels) < 10) {
  for (i in 1:length(rand.idcs)) {
    i <- rand.idcs[i]
    label <- thinned.ints[[i]]$label
    if (!(label %in% test.labels)) {
      print(paste('adding label:', label))
...
```{r}
zero.train <- training.set[[which(sapply(training.set, function(dig) { dig$label == 0 }))]]
one.train <- training.set[[which(sapply(training.set, function(dig) { dig$label == 1 }))]]
# create the path
zero.train <- generatePaths(list(zero.train), animation = TRUE)
one.train <- generatePaths(list(one.train))

# FIXME TO BIG LARGEST STROKE

# create step distributions
# there should also be something to do with the number of steps, 41 for this zero
(zero.total.steps <- length(zero.train[[1]]$direction.steps[[1]]))
(zero.train.step.probabilities <- rep(0, 8))
for (step.type in 1:8) {
  # find number of times step happened in the first stroke
  zero.train.step.probabilities[step.type] <- sum(zero.train[[1]]$direction.steps[[1]] == step.type)/zero.total.steps
}
fractions(zero.train.step.probabilities)

one.steps <- one.train[[1]]$direction.steps[[which.max(sapply(one.train[[1]]$direction.steps, length))]]
(one.total.steps <- length(one.steps))
(one.train.step.probabilities <- rep(0, 8))
for (step.type in 1:8) {
  # find number of times step happened in the first stroke
  one.train.step.probabilities[step.type] <- sum(one.train[[1]]$direction.steps[[1]] == step.type)/one.total.steps
}
fractions(one.train.step.probabilities)
```
