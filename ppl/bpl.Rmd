...
# what is the range of total steps
steps.sd <- sd(sapply(train.objects, function(obj) { obj$total.steps }))
...
first.steps <- rep(0, 8)
for (i in 1:8) {
  first.steps[i] <- sum(sapply(train.objects, function(obj) { obj$first.major.step == i }))
}
first.steps.dist <- first.steps/10

second.steps <- rep(0, 8)
for (i in 1:8) {
  second.steps[i] <- sum(sapply(train.objects, function(obj) {
    ss <- obj$second.major.step
    ifelse(is.na(ss), FALSE, ss == i)
  }))
}
second.steps.dist <- second.steps/10
...
for (i in 1:length(test.set)) {
  ...
  res <- lapply(train.objects, function(train.obj) {
    ...
    length.prob <- ifelse(abs(train.obj$total.steps - estimate$total.steps) <= step.range.threshold, 0.9, 0.1)
    strokes.prob <- ifelse(train.obj$num.real.strokes == estimate$num.real.strokes, 0.9, 0.1)
    # weight by changes in direction
    changes.prob <- ifelse(abs(train.obj$changes.in.direction - estimate$changes.in.direction) <= changes.threshold, 0.9, 0.1)
    ...
    first.major.step.prob <- if ((!is.na(estimate$first.major.step)) &&
      ...
      # THIS COULD BE 0
      max(0.1*first.steps.dist[estimate$first.major.step], 0.1)
    }

    # FIXME: if both NA this should be a greater probability
    second.major.step.prob <- if ((!is.na(estimate$second.major.step)) &&
                                  (!is.na(train.obj$second.major.step)) &&
                                  (train.obj$second.major.step == estimate$second.major.step)) {
      first.steps.dist[estimate$first.major.step]
    } else {
      # THIS COULD BE 0
      max(0.1*first.steps.dist[estimate$first.major.step], 0.1)
    }
    return(strokes.prob*first.major.step.prob*second.major.step.prob*length.prob*changes.prob)
  })
  (pred <- which.max(res)-1)
  if (length(pred) > 1) print('more than one prediction')
  #print(paste('predicting a:', pred))
  actual <- test.digit$label
  #print(paste('actual is a:', actual))
  #Sys.sleep(0.2)
  # random selection
  if (all(is.na(res))) {
...
# got 32.22% with the following training idcs
# [1] 1765 2183 3251 1843 5043 2380 2520 5291  486 5079
...
for (iter in 1:ntests) {
  random.draws <- which(rmultinom(90, size = 1, prob = apriori.test.probs) == 1, arr.ind = TRUE)[,1]
  #hist(random.draws)
